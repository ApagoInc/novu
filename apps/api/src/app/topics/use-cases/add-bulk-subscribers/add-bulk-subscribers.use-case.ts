...
import { AddBulkSubscribersCommand } from './add-bulk-subscribers.command';
...
@Injectable()
export class AddBulkSubscribersUseCase {
  constructor(
  ...
  async execute(
    command: AddBulkSubscribersCommand
  ): Promise<Array<Omit<ISubscriberGroups, 'subscribersAvailableToAdd'> & { key: string }>> {
    const results: Array<Omit<ISubscriberGroups, 'subscribersAvailableToAdd'> & { key: string }> = [];

    await Promise.all(
      command.topicKeys.map(async (topicKey) => {
        let topic = await this.topicRepository.findTopicByKey(topicKey, command.organizationId, command.environmentId);
    ...
    if (command.removeKeys)
      await Promise.all(
        command.removeKeys.map(async (topicKey) => {
  ...
  private async filterExistingSubscribers(command: AddBulkSubscribersCommand): Promise<ISubscriberGroups> {
